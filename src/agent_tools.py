...
def build_tool_context(
    ...
    *,
    ...
    primary_root = config.cwd.resolve()
    return ToolExecutionContext(
        root=primary_root,
        command_timeout_seconds=config.command_timeout_seconds,
        max_output_chars=config.max_output_chars,
        permissions=config.permissions,
        allowed_roots=tuple([primary_root] + [p.resolve() for p in config.additional_working_directories]),
...
def _resolve_path(raw_path: str, context: ToolExecutionContext, *, allow_missing: bool = True) -> Path:
    ...
    roots = context.allowed_roots if context.allowed_roots else (context.root,)
